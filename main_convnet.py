...
from net import ConvNet, DoubleConvNet
...
if os.path.exists('checkpoint'):
    if os.path.exists('checkpoint/ckpt_convnet.pth'):
        print('Checkpoint exists, reading weights from checkpoint...')
        checkpoint = torch.load('checkpoint/ckpt_convnet.pth')
        ...
        if step % 100 == 0:
            ...
            with torch.no_grad():
                ...
                if decoded.device != 'cpu':
                    decoded = decoded.squeeze().to('cpu')
                for i in range(N_TEST):
                    a[1][i].clear()
